...
- Content version `0.4.0`, last updated 2026-09-22 (see "Review of
  2026-09-22" below). The Arabic, transliterations and enumeration are
  unchanged from the 2026-09-13/2026-09-21 audit passes; 12 English meanings
  and all 99 explanations changed in this pass.
- All 99 Names are now `"verificationStatus": "reviewed"` (previously
  `"pending"`). None are `"verified"` — that status requires a second,
  independent review and has not happened. See "Review of 2026-09-22" for
  exactly who reviewed this pass and on what basis.
- Audit result (2026-09-13/2026-09-21 audit, unchanged): 72 passed without
  changes, 3 corrected, 25 flagged for review — see "Review of 2026-09-22"
  for how those 25 were resolved.
- Every entry carries an `audit.evidenceCategory` (Q/QH/H/M-Q/M-QH/E, see
  "Second audit pass" below) — a stricter, separate classification of how
  directly the Name itself is attested. It does not override `audit.result`.
  Names in category `E` or `M-Q`/`M-QH` now show a short evidence note on
  their Name page (`lib/content/evidence.ts`).
- All 99 Names now have a learner-facing `explanation`. There is still no
  pronunciation audio; recording and review are not yet scheduled (PLAN.md Phase 10).
...
A spreadsheet (`99-names-scholar-review-updated.xlsx`, three sheets: Scholar
review, Review decisions, Wording review) was supplied as a further
AI-assisted research pass over the audit above. **The workbook itself
records that it is not a completed qualified-scholar review**: its own
"Review decisions" sheet states "scholar approval is pending" and that the
human-decision columns (`correct_true_false`, `scholar_explanation`) were
left blank for the reviewer. No separate qualified scholar signed off on it.

The project owner (Mohammed, 2026-09-22) reviewed the workbook's proposals
and explicitly directed that they be treated as approved and applied. That
is the basis recorded here for every change below —
**an owner-approval pass building on two AI-assisted audits, not
independent outside-scholar review.** `verificationStatus` was moved from
`"pending"` to `"reviewed"` accordingly (one review pass). No entry is
`"verified"` (that requires a second, independent review).

### What changed

1. **Explanations.** All 99 Names now have a learner-facing `explanation`,
   drawn from the workbook's "Short explanation" column (editorial drafts
   based on the audit's meanings and notes, not independently generated by
   this pass).
2. **12 priority wording changes**, applied to `shortMeaning` (all other
   "Additional audit wording" suggestions in the workbook were left
   unapplied and remain documented proposals only, consistent with this
   project's rule against silently resolving a documented disagreement):

   | #   | Name        | Old                   | New                                                      |
   | --- | ----------- | --------------------- | -------------------------------------------------------- |
   | 5   | As-Salām    | The Source of Peace   | The Perfect, Giver of Peace                              |
   | 7   | Al-Muhaymin | The Guardian Over All | The Overseer and Guardian                                |
   | 9   | Al-Jabbār   | The Compeller         | The Supreme Compeller and Restorer                       |
   | 18  | Al-Fattāḥ   | The Opener            | The Opener and Supreme Judge                             |
   | 25  | Al-Mudhill  | The Giver of Dishonor | The One Who Brings Low / Removes Honor                   |
   | 39  | Al-Muqīt    | The Nourisher         | The Sustainer and Nourisher                              |
   | 40  | Al-Ḥasīb    | The Reckoner          | The Sufficient Reckoner                                  |
   | 63  | Al-Qayyūm   | The Self-Subsisting   | The Self-Subsisting Sustainer                            |
   | 77  | Al-Wālī     | The Patron            | The One Who Governs All Affairs                          |
   | 81  | Al-Muntaqim | The Avenger           | The Executor of Just Retribution                         |
   | 91  | Aḍ-Ḍārr     | The Distresser        | The One in Whose Decree Harm Occurs / Controller of Harm |
   | 92  | An-Nāfiʿ    | The Benefactor        | The Bestower of Benefit                                  |

   The three 2026-09-13 corrections (Al-Wājid, Al-Muqtadir, Dhul-Jalāli
   wal-Ikrām) are retained unchanged.

3. **Pairing.** `lib/content/schema.ts` adds an optional `pairings` array
   (`withId` or `withLabel`, `mode`: `strong` | `teaching` | `companion`,
   `note`). Each Name stays its own numbered card — FSRS and IndexedDB card
   identity are untouched — but a `PairingNote` component now shows the
   paired Name and a short note on the Name detail page and in the Learn and
   Review cards (after reveal):
   - **strong** (never shown alone): Al-Muʿizz ↔ Al-Mudhill,
     Aḍ-Ḍārr ↔ An-Nāfiʿ, Al-Muntaqim ↔ Al-ʿAfuww.
   - **teaching** (non-disruptive link): Al-Qābiḍ ↔ Al-Bāsiṭ,
     Al-Khāfiḍ ↔ Ar-Rāfiʿ, Al-Mubdiʾ ↔ Al-Muʿīd, Al-Muḥyī ↔ Al-Mumīt,
     Al-Muqaddim ↔ Al-Muʾakhkhir.
   - **companion** (not one of the 99, not renumbered): Al-Māniʿ ↔
     Al-Muʿṭī (the Giver).
4. **Evidence labels.** `lib/content/evidence.ts` shows a short note on the
   Name detail page for `evidenceCategory` `E` (exact form rests on the
   enumeration alone) and `M-Q`/`M-QH` (Qur'anic evidence only in
   construct/restricted form). `Q`/`QH`/`H` entries show no note.
5. **Enumeration.** Confirmed and documented (Sources page, "The list this
   app follows"): Al-Aḥad is retained at app No. 67; the app describes its
   list as a documented classical traditional recension (al-Khaṭṭābī's
   _Shaʾn al-Duʿāʾ_), not a verbatim transcription of Tirmidhī 3507 or any
   other single narration. No numbering changed.
6. **The 25 flagged Names.** Kept in the enumeration with their existing
   `audit.result` (historical AI-audit finding, unchanged) and
   `verificationStatus` now `"reviewed"`; their disputed/methodology-dependent
   status is now shown to learners via the evidence label (item 4) and, where
   applicable, the pairing note (item 3), rather than hidden or silently
   resolved.

### Not done in this pass

- **Independent qualified-scholar peer review.** This remains outstanding;
  see "Basis" above. `verified` status requires it.
- **Pronunciation audio.** None was supplied; recording and review remain
  not yet scheduled (PLAN.md Phase 10).
- The workbook's other ~40 "Additional audit wording" suggestions were
  deliberately left unapplied (see item 2).

### Keeping the workbook out of the public repository

The supplied `.xlsx` and its companion `.csv` are working inputs, not public
content, and are excluded from version control (`.gitignore`). The decisions
extracted from them are recorded in this file instead.

## Launch readiness

Phase 10 is **substantially reviewed but not complete**. As of the
2026-09-22 pass: the Arabic, order, transliterations and English meanings
have gone through one review (the project owner's, building on two
AI-assisted audits — see "Review of 2026-09-22"), all 99 Names have a
learner-facing explanation, and the enumeration edition (al-Aḥad) is
confirmed and documented. What remains:

- an **independent, qualified-scholar review** — this pass was the project
  owner's own approval of an AI-assisted workbook, not outside peer review;
  PLAN.md's bar for public launch is not yet met;
- a **second review** for any entry to become `"verified"` rather than
  `"reviewed"`;
- **pronunciation audio** — not written or recorded; not yet scheduled.
